...
### 第二步：主会话直接执行，用例多时并行分片
本技能逻辑线性、体量小，**用例不多（≤8 条）时在主会话中直接完成**，无需 Agent 子会话。
**用例较多（>8 条）时按第六步「并行分片」拆 2~3 个并行子会话**加速生成，每次向用户报告进度。
...
6. 分批生成（>8 条）时，**同页面片段跨批复用**：第二批不再重写已生成的 page 片段，只补新用例的
