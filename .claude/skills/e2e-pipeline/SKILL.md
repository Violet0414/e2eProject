...
1. 先读取技能定义文件：/home/mxr/e2eProject/.claude/skills/requirement-split/SKILL.md，理解其中的处理规则和输出格式
2. 对以下需求内容进行拆分：

{用户传入的需求内容}

3. 严格按照技能定义的处理规则处理
4. 将结果写入输出文件：/home/mxr/e2eProject/output/{当前日期}/功能点列表.md（如目录不存在则先创建）
```
...
1. 先读取技能定义文件：/home/mxr/e2eProject/.claude/skills/test-point-extract/SKILL.md，理解其中的处理规则和输出格式
2. 读取输入文件：/home/mxr/e2eProject/output/{当前日期}/功能点列表.md
3. 读取参考文件：/home/mxr/e2eProject/referenceDocument/testPointsRequirement.md
4. 严格按照技能定义的处理规则，基于功能点列表和参考文件提取测试点
5. 将结果写入输出文件：/home/mxr/e2eProject/output/{当前日期}/测试点清单.md（如目录不存在则先创建）
```
...
1. 先读取技能定义文件：/home/mxr/e2eProject/.claude/skills/test-point-review/SKILL.md，理解其中的处理规则和输出格式
2. 读取输入文件：/home/mxr/e2eProject/output/{当前日期}/测试点清单.md
3. 读取功能点列表用于交叉比对：/home/mxr/e2eProject/output/{当前日期}/功能点列表.md
4. 读取参考文件：/home/mxr/e2eProject/referenceDocument/testPointsRequirement.md
5. 严格按照技能定义的处理规则进行评审
6. 将结果写入输出文件：/home/mxr/e2eProject/output/{当前日期}/测试点评审报告.md（如目录不存在则先创建）
```
...
1. 先读取技能定义文件：/home/mxr/e2eProject/.claude/skills/test-case-generate/SKILL.md，理解其中的处理规则和输出格式
2. 读取输入文件：/home/mxr/e2eProject/output/{当前日期}/测试点清单.md
3. 读取参考文件：/home/mxr/e2eProject/referenceDocument/testPointsRequirement.md
4. 严格按照技能定义的处理规则，基于测试点清单和参考文件生成测试用例
5. 将结果写入输出文件：/home/mxr/e2eProject/output/{当前日期}/测试用例.md（如目录不存在则先创建）
```
...
1. 先读取技能定义文件：/home/mxr/e2eProject/.claude/skills/export-excel/SKILL.md，理解其中的处理规则和输出格式
2. 读取输入文件：/home/mxr/e2eProject/output/{当前日期}/测试用例.md
3. 严格按照技能定义的处理规则，将测试用例导出为Excel文件
4. 将结果写入输出文件：/home/mxr/e2eProject/output/{当前日期}/testcases_{当前日期}.xlsx
```
