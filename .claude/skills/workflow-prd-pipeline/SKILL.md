...
6. route_path 处理：需求文档中已明确路由的模块直接提取；无路由的模块禁止猜测，也不要直接采用探索记录中的路由，对应用例的 route_path 置空；最终回复中单独列出所有 route_path 为空的用例所属模块清单（去重），并附上探索记录中的候选路由（如有）供主会话向用户确认
```
...
5. **评审拦截**：步骤3（测试点评审）完成后，读取评审报告：
   ...
   - 如评审结论为"通过"：继续执行步骤4
6. **route_path 确认（步骤4之后、步骤5之前）**：步骤4完成后，主会话读取测试用例.md，将 route_path 为空的用例按「所属模块」归组，向用户确认各模块的 route_path：
   - 探索记录中存在候选路由时，作为建议值一并展示，**经用户确认后**方可使用（不得未经确认直接采用）
   - 用户提供或确认候选值 → 回填到该模块所有 route_path 为空的用例，再继续步骤5
   - 用户不提供 → 该模块用例 route_path 保持置空，直接继续步骤5
   - 全部用例 route_path 均非空时跳过本步
7. **完成报告**：全部步骤完成后，向用户汇报流水线执行结果，列出所有输出文件路径
