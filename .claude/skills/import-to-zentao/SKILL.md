...
### 步骤5 生成提交清单 + dry-run
把 `cases.json` 每条用例映射为提交对象（务必含三数组，见下）。**title 必须带模块前缀：`<所属模块>-<用例名称>`**，例如用例名称 `[列表查询】无条件查询展示全部资产`、所属模块 `全维度资产台账` → title `全维度资产台账-[列表查询】无条件查询展示全部资产`。前缀模块名取自用例文件"所属模块"列的**最后一级**（不含上级路径）。**防重复**：若用例名称已以 `模块名-` 开头（test-case-generate / generate-testcases-from-explore 生成时已带前缀），直接使用原名称作为 title，**禁止二次拼接**；判断时先比对用例名称是否以"所属模块最后一级 + `-`"开头：
```json
{
