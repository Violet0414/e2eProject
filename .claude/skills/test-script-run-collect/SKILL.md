...
## 执行结果
| 用例ID | 模块 | 状态 | 名称 | 失败原因 | bug标题 | 重现步骤 | 结果 | 预期 | 严重程度 | 优先级 | 截图 |
## 失败用例详情
| 用例ID（错误堆栈 + 截图路径）
```
- 状态列：`通过` / `失败` / `通过(重跑)`，一眼判断。
- 失败原因列：系统实际行为 vs 用例预期 → 如实写"系统实际 X vs 用例预期 Y"（作测试发现，**不改用例预期**）；脚本异常/超时 → 写脚本报错要点。
- **bug 字段列（失败行填充，通过行显示"-"）**，供 bug-to-zentao / 人工提 bug 直接取用：
  - `bug标题` / `重现步骤` / `预期`：优先从 `测试用例.md`（`--case-doc` 或按批次日期自动匹配 `output/*/测试用例.md`）按用例编号反查真实步骤与预期结果；找不到时降级按 bug-to-zentao 规则从用例名称拼接（标题取用例名/补【模块】前缀，步骤/预期取"验证"后文本）。
  - `结果`：从 traceback 提取最终断言/异常消息行（截断 200 字）。
  - `严重程度` / `优先级`：按用例文档优先级映射（高→1、中→2、低→3），无文档时按 bug-to-zentao 的 SEV1/SEV3 特征规则判定，默认 2；`优先级=严重程度`。
  - `截图`（即 bug截图）：沿用失败截图列。
...
| `--case-doc` | 自动匹配 | 测试用例.md 路径（bug 字段列数据源）；缺省按批次日期/最新批次找 `output/*/测试用例.md`，找不到降级按用例名称拼接 |

## 校验清单
- [ ] `run_collect.py` 通过 `python -m py_compile`；`--help` 参数完整
- [ ] 每个脚本被实际执行，结果协议行已捕获，统计与清单准确
- [ ] `测试报告.md` 已生成，含执行结果表，每脚本状态明确（通过/失败/通过(重跑)）+ 失败用例详情
- [ ] 失败行 bug 字段列（bug标题/重现步骤/结果/预期/严重程度/优先级）已填充且非"-"；通过行显示"-"
