...
6. **【强制】每个元素必须先检查 data-testid**：探索任何交互元素（输入框、下拉、按钮、单选/多选、日期控件等）时，必须先检查是否存在 `data-testid` 属性（quick 模式在批量采集阶段一次性提取，full 模式逐个用 `page.locator(...).get_attribute('data-testid')` 检查）。若存在，**必须**将 `[data-testid="xxx"]` 作为首选定位器记录在"元素定位选择器"列的第一行。若发现 testid 不符合命名规范（非 kebab-case、页面内重复、疑似动态值），如实在探索记录「备注」列标注，供后续按 `files/templates/testid_naming_convention.md` 反馈前端修复。
...
**探索记录文件**：`{项目目录}/explore_output/{当前日期}_{时间}/explore_record.md`，结构如下：
...
2. **截图保存到 `{项目目录}/explore_output/{当前日期}_{时间}/screenshots/`（按需截图，文件名格式：`{序号}_{描述}.png`）
...
- 两模式输出的 `explore_record.md` 结构完全一致，均满足后续 generate-testcases-from-explore 的输入要求
